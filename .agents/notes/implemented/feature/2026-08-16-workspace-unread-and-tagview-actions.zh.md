...
**从行创建的会话继承来源行分组。** `startSession` 会解析出它实际打开的空白会话。`SessionTree` 与 `FlatList` 将该 id 直接分配到来源行的分组，不依赖当前会话通知时序，也不受后续导航影响。
...
三个工作区视图共享会话创建与未读行为，分组展开状态可跨刷新和工作区变化保留，深链也能显示所选行。这些偏好仍局限于浏览器 profile。即使复用的空白会话已经是当前项，或创建完成前发生了导航，从行创建的会话仍会进入所选分组。
