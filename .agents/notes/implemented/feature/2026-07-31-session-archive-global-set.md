...
- UI: the `delete` menu row (visual-only) becomes `archive` (label "Archive session", non-danger styling, no confirmation dialog); row and bulk controls use the local two-click confirmation owned by [workspace bulk archive selection](2026-08-20-workspace-bulk-archive-selection.md). Filtering is one extra arm in `tree.ts`'s `sessionVisible` predicate, with `deriveGroups`/`deriveFlat` taking an `archived` set parameter so all four surfaces (group loop, stray bucket, search, flat) share one source.
